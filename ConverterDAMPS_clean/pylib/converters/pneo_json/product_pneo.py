...
class Product_Pneo(Product_Directory):
    xmlMapping = {
        ...
        # [INFERRED from Pleiades DIMAP layout] not in the original pneo
        # extraction; the PLN template requires illumination angles. Real-TDS
        # confirmation PENDING that PNEO DIMAP carries Solar_Incidences.
    ...
    # eop: ascendingNodeLongitude, eop: acquisitionStation
    optionnal_nodes = []
    ...
    def refineMetadata(self, xmlHelper_: xmlHelper.XmlHelper):
        ...
        self.metadata.setMetadataPair(
            ...
            format(float(self.metadata.getMetadataValue(metadata.METADATA_RESOLUTION)), '.2f')
        )
        self.metadata.setMetadataPair(
