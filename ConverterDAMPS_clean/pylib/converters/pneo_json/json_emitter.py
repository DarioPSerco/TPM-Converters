"""JSON metadata emitter for PLEIADES NEO products.

Mission adapter for the common template-driven generator
(``common/json_template.py``). Keeps PNEO's own extraction (the metadata
dict populated by ``product_pneo.Product_Pneo.extractMetadata``), declares
the PLEIADES NEO mission-fixed value map, and hands both layers to the
mission-agnostic generator. Output matches ``TDS/template/PLN-template.json``
— structure and values.

Same Airbus-DIMAP template family as Pleiades (PL1/PLN key-paths are
identical; only 3 mission strings differ), so this mirrors
``pleiades_json/json_emitter.py`` with a mission-local placeholder template
(``feature_template_pln.json``) through the generator's existing
``template_path`` extension point — no change to the common core.

NOTE: instrumentShortName "HiRI" comes from the PLN template (the ONLY
structural reference). It is suspected wrong (HiRI is the Pleiades 1A/1B
instrument; the cfg says PNEO) — see PENDING in PLEIADES_PNEO_DEV.md.
...
from pneo_json import __version__

TEMPLATE_PATH = Path(__file__).resolve().parent / "feature_template_pln.json"

NO_NUMERIC_VALUE = -999999.0

# PLEIADES NEO mission-fixed values — template slots constant for this
# mission, shaped as a partial tree mirroring the template.
MISSION_VALUES = {
    "properties": {
        "acquisitionInformation": {
            "platform": {"platformShortName": "PLEIADES NEO", "orbitType": "LEO"},
            "instrument": {"instrumentShortName": "HiRI", "sensorType": "OPTICAL"},
        },
        "productInformation": {
            "resourceLineage": {"processStep": {
                "description": "EOPF-EOS Converter for PLEIADES NEO",
                "reference": {"title": "EOPF-EOS Specialization for PLEIADES NEO products",
                              "edition": "1.0"},
                "processingInformation": {"softwareReference": {
                    "title": "EOPF-EOS Converter for PLEIADES NEO",
                    "edition": __version__}},
            }},
        },
    },
...
def build_layers(met, product, eo_product_name, native_product_name=None):
    """PLEIADES NEO per-product dynamic values, from this mission's extraction."""
    ...
    # sun angles: extraction paths [INFERRED from Pleiades DIMAP layout],
    # see product_pneo.xmlMapping — real-TDS confirmation PENDING
