...
class SipMessageBuilder(SipBuilder):
    ...
    def addToSipMessage(self, mess, segment, met=None, optional=False):
        """Add to sip message, resolve '@xxx@' and
        '$$self.getNextCounter()$$' tags
        """
        if mess is not None:
            if self.debug != 0:
                print("@@@@###@@@\n@@@@###@@@@\n@@@@###@@@ addToSipMessage: segment='%s'. optional=%s" % (
                    segment, optional)
                )
            pos = segment.find('@')
            if pos < 0:
                ...
                if tmp.find(sipBuilder.VALUE_UNKNOWN) > 0:
                    if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' is UNKNOWN 0. optional=%s" % (
                    segment, optional))
                    found = False
                if tmp.find(sipBuilder.VALUE_NONE) > 0:
                    if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' is NONE 0. optional=%s" % (
                    segment, optional))
                    found = False
                if tmp.find(sipBuilder.VALUE_NOT_PRESENT) > 0:
                    if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' is NOT_PRESENT 0. optional=%s" % (
                    segment, optional))
                    found = False

                #
                if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' result 0: found=%s" % (
                segment, found))
                if found:
                    return "%s%s" % (mess, tmp)
                elif optional == False:
                ...
                found = True
                if tmp.find(sipBuilder.VALUE_UNKNOWN) > 0:
                    if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' is UNKNOWN 1. optional=%s" % (
                    segment, optional))
                    found = False
                if tmp.find(sipBuilder.VALUE_NONE) > 0:
                    if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' is NONE 1. optional=%s" % (
                    segment, optional))
                    found = False
                if tmp.find(sipBuilder.VALUE_NOT_PRESENT) > 0:
                    if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' is NOT_PRESENT 1. optional=%s" % (
                    segment, optional))
                    found = False

                #
                if self.debug != 0: print("@@@@@@@@@@@\n@@@@@@@@@@@@@@\n@@@@@@@@@@@@@ field '%s' result 1: found=%s" % (
                segment, found))
                if found:
                    return "%s%s" % (mess, tmp)
                elif optional == False:
    ...
    def getRepresentationUsed(self, metadata):
        ...
        try:
            ...
            if self.debug != 0:
                print("### getRepresentationUsed: final representation for %s:'%s' used:'%s'" % (
                self, representation_name, res))
        except:
            res = self.__getattribute__(sipBuilder.TYPOLOGY_DEFAULT_REPRESENTATION)
            if self.debug != 0:
                print("### getRepresentationUsed: WARNING: typology '%s' not available in %s; use:'%s'" % (
                representation_name, self, sipBuilder.TYPOLOGY_DEFAULT_REPRESENTATION))
    ...
    def _buildMessage(self, metadata, currentTreePath):
        ...
        if self.debug != 0:
            print("  _buildMessage: thisToUse=%s" % thisToUse)
        ...
        if len(thisToUse) > 0:
            ...
            if self.debug != 0:
                print("  _buildMessage:thisToUse is empty")
        ...
        for field in representationToUse:
            ...
            if used == 1:
                ...
                if field.strip()[0] == '<':
                    ...
                    if conditions != None:
                        try:
                            key = list(conditions.keys()).index(field)
                            condOk = False
                    ...
                    if condOk:
                        ...
                        # print "piecesDir is:%s" % piecesDir
                        full_module_path = '.'.join(__name__.split('.')[:-1] + [field])
                        module = __import__(full_module_path, fromlist=(field, ))
                        print(f"DEBUGGING:: {full_module_path} : {field}")
