...
#
from eoSip_converter.esaProducts import metadata, base_metadata
...
class NamingConvention_Dd(NamingConvention):
    #
    # Aeolus pattern
    DD_PATTERN = '<SSS>_<CCCC>_<TTTTTTTTTT>_<instance ID>.<extension>'
    DD_PATTERN_INSTANCE = '<yyyymmdd>_<YYYYMMDD>'
    DDv_PATTERN_INSTANCEv2 = '<yyyymmdd>_<YYYYMMDD>_<vv>'
    DDv_PATTERN_INSTANCEv2V4 = '<yyyymmdd>_<YYYYMMDD>_<vv>_v<VVVV>'

    #
    DD_POSSIBLE_PATTERN = [DD_PATTERN_INSTANCE, DDv_PATTERN_INSTANCEv2, DDv_PATTERN_INSTANCEv2V4]
    ...
    #
    #
    def __init__(self, p=DD_PATTERN_INSTANCE, fromSuper=False):
        NamingConvention.__init__(self, p, True)
        # print "## NamingConvention_Dd init: pattern=%s; fromSuper=%s" % (p, fromSuper)

        # the possible pattern used
        for item in self.DD_POSSIBLE_PATTERN:
            try:
        ...
        if self.debug != 0:
            print(" #### NamingConvention_Dd p=%s" % p)

        if p[0] != '<':
            if self.debug != 0:
                print("NamingConvention_Dd init case 0: pattern=%s" % p)
            self.usedPattern = eval("NamingConvention_Dd.%s" % p)
            self.usedBase = NamingConvention_Dd.DD_PATTERN
    ...
    def buildProductName(self, met=None, ext=None):
        ...
        # os._exit(1)
        if self.debug != 0:
            print("\n\n NamingConvention_Dd.buildProductName, pattern used:%s, ext:%s" % (self.usedPattern, ext))
    ...
    def buildInstance(self, met=None):
        ...
        for tok in self.usedPattern.split('_'):
            ...
            if tok == '<yyyymmdd>':
                ...
                tmp = self.buildToken(met, metadata.METADATA_PRODUCT_VERSION, 2, '#')
                res = "%s_%s" % (res, tmp)
                if self.debug != 0:
...
if __name__ == '__main__':
    ...
    try:
        n = NamingConvention_Dd(NamingConvention_Dd.DDOT_PATTERN_INSTANCE)
        n2 = NamingConvention_Dd(NamingConvention_Dd.DDOTvV_PATTERN_INSTANCE)
