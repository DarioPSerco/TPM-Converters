...
class NamingConvention_Aeolus(NamingConvention):
    ...
    def __init__(self, p=None, fromSuper=False):
        if p is None:
            p = self.EOLUS_PATTERN_INSTANCE_GENERIC_DTOV
    ...
    def buildProductName(self, met=None, ext=None):
        ...
        for tok in toks:
            ...
            if tok == '<AE>':
                res = formatUtils.normaliseNumber(met.getMetadataValue(metadata.METADATA_PLATFORM), len(tok) - 2, None,
                                                  1).upper()
                if self.debug != 0:
