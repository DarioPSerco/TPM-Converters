...
class Ingester(Base_Ingester, ABC):
    ...
    def changeLogFileHandler(self, logFilePath):
        ...
        try:
            ...
            # remove old log file: only the CWD bootstrap converter*.log, never a per-run log
            if logPath is not None:
                try:
                    self.file_handler.close()
                    if os.path.basename(logPath).startswith(CONVERTER_LOG_FILE_NAME):
                        os.remove(logPath)
                except:
                    exc_type, exc_obj, exc_tb = sys.exc_info()
                    print(
    ...
    #
    # per-run log context: log/<mission>/<YYYY-MM-DD>/<mission>_<YYYYMMDDTHHMMSS>_<pid>_*
    # captured ONCE per run so all files of a run share the same stamp and dated folder
    #
    def initRunContext(self):
        now = time.localtime()
        self.runDate = time.strftime("%Y-%m-%d", now)
        self.runStamp = time.strftime("%Y%m%dT%H%M%S", now)
        self.runDir = "%s/%s/%s" % (LOG_FOLDER, self.MISSION_ID, self.runDate)
        if not os.path.exists(self.runDir):
            os.makedirs(self.runDir)
        self.runPrefix = "%s/%s_%s_%s" % (
            self.runDir,
            self.MISSION_ID,
            self.runStamp,
            os.getpid(),
        ...
        basicFormat = (
            "%%(asctime)s - [%%(levelname)s] [%s] : %%(message)s" % self.MISSION_ID
        )
        self.formatter = logging.Formatter(basicFormat)
    ...
    def readConfig(self, path=None):
        ...
        try:
            ...
            try:
                self.MISSION_ID = self.__config.get(SETTING_Main, SETTING_MISSION_ID)
            except:
                self.MISSION_ID = self.CONFIG_NAME
    ...
    def starts(self, args):
        ...
        # per-run log context: log/<mission>/<YYYY-MM-DD>/, one stamp per run
        if changeBatchName:
            print(" changed fixed_batch_name=%s" % self.fixed_batch_name)
        else:
            print(" fixed_batch_name=%s" % self.fixed_batch_name)
        self.initRunContext()
        self.file_toBeDoneList = "%s_product_list.txt" % self.runPrefix
        self.changeLogFileHandler("%s.%s" % (self.runPrefix, CONVERTER_LOG_FILE_EXT))
        self.logger.info(" run log folder:%s" % self.runDir)
    ...
    def writeCurrentProduct(self, aProcessInfo):
        ...
        try:
            apath = "%s_current.txt" % self.runPrefix
            fd = open(apath, "w")
    ...
    def writeDoneProducts(self, aProcessInfo=None):
        ...
        try:
            apath = "%s_DONE.txt" % self.runPrefix
            fd = open(apath, "w")
    ...
    def writeFailledProducts(self, aProcessInfo=None):
        ...
        try:
            apath = "%s_ERROR.txt" % self.runPrefix
            fd = open(apath, "w")
    ...
    def writeKeepedInfo(self, keeper=None, aProcessInfo=None):
        ...
        try:
            ...
            apath = None
            if keeper == None:
                apath = "%s_KEEPED.txt" % self.runPrefix
                fd = open(apath, "w")
                fd.write(self.infoKeeper.toString())
            else:
                apath = "%s_KEEPED2.txt" % self.runPrefix
                fd = open(apath, "w")
    ...
    def writeLogInfo(self, mess, aProcessInfo=None):
        ...
        try:
            apath = "%s_log.txt" % self.runPrefix
            fd = open(apath, "w")
            fd.write(mess)
            fd.flush()
            fd.close()
    ...
    def writeRunMarker(self):
        """Write the SUCCESS/FAILED run marker in the run log folder"""
        try:
            status = "FAILED" if self.num_error > 0 else "SUCCESS"
            apath = "%s_%s" % (self.runPrefix, status)
            fd = open(apath, "w")
            fd.write(
                "batch:%s\ntotal:%s\ndone:%s\nerror:%s\n"
                % (self.batchName, self.num_total, self.num_done, self.num_error)
            )
            fd.flush()
            fd.close()
            print(" run marker written in:%s" % apath)
        except Exception as eee:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            print("Error: problem writing run marker")
            print(
                " problem is:%s  %s\n%s\n" % (exc_type, exc_obj, traceback.format_exc())
            )
    ...
    def makeConversionReport(self, label, path=None):
        """
        Label is written at report start: like "Image2006 IrsP6 conversion report"
        and is the report name; without a path the report goes in the run log folder
        """
        out = StringIO()
        ...
        if path is None:
            reportPath = "%s_report.txt" % self.runPrefix
        else:
    ...
    def makeJsonShape(self):
        ...
        jsonPath = "%s_convertedItems.json" % self.runPrefix
