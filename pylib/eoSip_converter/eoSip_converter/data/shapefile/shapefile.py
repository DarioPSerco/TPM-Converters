...
class _Record(list):
    ...
    def as_dict(self):
        ...
        :return: dict
        """
        return dict((f, self[i]) for f, i in list(self.__field_positions.items()))
...
class Reader(object):
    ...
    def __geo_interface__(self):
        ...
        for feat in self.iterShapeRecords():
            fdict = {'type': 'Feature',
                     'properties': dict(list(zip(fieldnames, feat.record))),
...
class Writer(object):
    ...
    def __init__(self, target=None, shapeType=None, autoBalance=False, **kwargs):
        print((" Writer.__init__: target=%s; shapeType=%s; autoBalance=%s; kwargs=%s" % (
        target, shapeType, autoBalance, kwargs)))
        self.target = target
    ...
    def record(self, *recordList, **recordDict):
        ...
        will be added."""
        if debug:
            print(("## record: recordList=%s; recordDict=%s" % (recordList, recordDict)))
        ...
        if debug:
            print(("## record: fieldCount=%s; recordList=%s" % (fieldCount, len(recordList))))
            n = 0
            for item in self.fields:
                print((" fields[%s]=%s" % (n, item)))
                n += 1
...
def test(**kwargs):
    ...
    with open("README.md", "rb") as fobj:
        test = doctest.DocTestParser().get_doctest(string=fobj.read().decode("utf8").replace('\r\n', '\n'), globs={},
                                                   name="README", filename="README.md", lineno=0)
