...
# for verification
REF_TYPECODE = {
    "PSC_DEF_OT", "PSC_DEF_S3", "PSC_DEF_S4",  # 3-band, 4-band and 5-band Analytic and Visual Ortho Tile level 3A
...
OBSOLETE_TO_NEW_TYPECODE_REQS = {
    ...
    "PSC_4AB_1B": {
        'assets': ("basic_analytic", "basic_udm2", "basic_analytic_rpc", "basic_analytic_xml"),
        'levels': ('L1B', 'L1A'),
        'bands': 4
    },
    "PSC_3VO_3B": {
...
class Product_Planetscope(Product_Directory):
    ...
    def extractToPath(self, folder=None, dont_extract=False):
        ...
        for root, dirs, files in os.walk(self.EO_FOLDER, topdown=False):
            ...
            for name in files:
                ...
                for ending in raw_browse_path_endings_order:
                    ...
                    for ending in raw_xml_endings_order:
                        if name.endswith(ending):
                            ...
                            shutil.copyfile(self.xml_metadata_path, "%s%s%s" % (folder, os.sep, name))
                ...
                if self.old_product_type_from_obsolete_native() is not None:
                    assets = [Path(p).name for p in Path(self.EO_FOLDER).iterdir() if Path(p).is_dir()]
                    found = False
                    for req_asset in ("analytic", "analytic_sr", "basic_analytic", "visual",):
                        for asset in assets:
                            if asset == req_asset:
    ...
    def product_type_from_obsolete_native(self):
        ...
        is_obsolete_prod_type = self.old_product_type_from_obsolete_native() is not None
        if best_prod_level == 'L3A' and is_obsolete_prod_type:
            raise RuntimeError(f"{self.EO_FOLDER.split(os.sep)[-1]}'s best product level is 'L3A'. No new product type equivalent")
        ...
        poss_types = [
            ...
            if all(req_asset in asset_dirs for req_asset in OBSOLETE_TO_NEW_TYPECODE_REQS[t]['assets'])
            and
    ...
    def determineTypeCode(self):
        ...
        for typecode in TYPECODES_IN_ORDER_OF_IMPORTANCE:
            ...
            if all(map(lambda req: req in assets, required_assets)):
                return typecode
