...
from .product_aeolus import Product_Aeolus
from xml_nodes import sipBuilder


class Product_Aeolus_Aux(Product_Aeolus):
    METADATA_NAME = '.EEF'
    EXTRACTED_PATH = None
    metadata_data = None
    metadata_path = None
    ...
    xmlMapping = {
        metadata.METADATA_START_DATE: 'Earth_Explorer_Header/Variable_Header/Main_Product_Header/Sensing_Start',
    ...
    ADV_ORDER = ['ADV_minRollAngle', 'ADV_maxRollAngle', 'ADV_minPitchAngle', 'ADV_maxPitchAngle', 'ADV_minYawAngle',
                 'ADV_maxYawAngle']
    ...
    def __init__(self, path, otherPath=None):
        Product_Aeolus.__init__(self, path, otherPath)  # otherPath is None
        self.debug = 1
        print(" init class Product_Aeolus_Aux: path=%s" % self.path)
    ...
    def refineMetadata(self):
        ...
        # get product type
        ptype = namingConventionEo.getFilenameElement(self.origName, NamingConvention_Aeolus.EOLUS_PATTERN,
                                                      'TTTTTTTTTT')
        ...
        self.metadata.setMetadataPair(metadata.METADATA_SOFTWARE_NAME,
                                      self.metadata.getMetadataValue(metadata.METADATA_SOFTWARE_NAME).replace(' ', ''))

        # set acquisitionType
        OTHER = ['ALD_U_N_0_', 'ALD_U_N_1A']  # OTHER
        NOMINAL = ['ALD_U_N_1B', 'ALD_U_N_2A', 'ALD_U_N_2B', 'ALD_U_N_2C']  # NOMINAL
        CALIBRATION = ['AUX_CAL_L2', 'AUX_CSR_1B', 'AUX_DCC_1A', 'AUX_DCC_1B', 'AUX_DCMZ1B', 'AUX_DMCZ1A', 'AUX_HBE_1B',
                       'AUX_IAT_1A', 'AUX_IAT_1B', 'AUX_IDC_1A', 'AUX_IDC_1B', 'AUX_IRC_1A', 'AUX_ISR_1A', 'AUX_ISR_1B',
                       'AUX_LCP_1A', 'AUX_LCP_1B', 'AUX_LDT_1A', 'AUX_LDT_1B', 'AUX_MRC_1B', 'AUX_NOU_1A', 'AUX_OWV_1A',
                       'AUX_OWV_1B', 'AUX_PRR_1B', 'AUX_RBC_L2', 'AUX_RRC_1B', 'AUX_ZWC_1B',
                       'AUX_RDB_1B']  # CALIBRATION
        ...
        # set FILE classe:
        tmp = self.metadata.getMetadataValue(metadata.METADATA_FILECLASS)[0] + self.metadata.getMetadataValue(
            'VARIOUS_HEADER_SYSTEM')[0:3]
        ...
        for field in self.xmlMapping:
            if field[0:4] == 'ADV_':
                ...
                if mapping.startswith(FIXED):
                    print(" ################################################################## handle ADV FIXED info: %s=%s" % (
                        field, mapping))
        ...
        # ascending node date
        self.metadata.setMetadataPair(metadata.METADATA_ASCENDING_NODE_DATE, "%sT%sZ" % (
    ...
    def extractSensorMode(self):
        ...
        LUT = {'ALD_U_N_1B': 'MEASUREMENT', 'ALD_U_N_2A': 'MEASUREMENT', 'ALD_U_N_2B': 'MEASUREMENT',
               'ALD_U_N_2C': 'MEASUREMENT',
               'AUX_CAL_L2': 'CAL',
               'AUX_IAT_1B': 'IAB',
               'AUX_CSR_1B': 'CSR',
               'AUX_DCC_1B': 'DCC',
