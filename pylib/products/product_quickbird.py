...
class Product_Quickbird(Product_Directory):
    ...
    def extractMetadata(self, met=None, processInfo=None):
        ...
        # if self.mulTilPath is not None:
        #    self.getMetadataFromIMD(processInfo)
        # else:
        #    self.getMetadataBis(processInfo)
    ...
    def refineMetadata(self, processInfo):
        ...
        changed, newRes = testResolution(tmp)
        if changed:
            processInfo.addLog(" !!!! INVALID resolution: %s; changed to: %s" % (tmp, newRes))
            self.metadata.setMetadataPair("INVALID resolution", "%s; changed to: %s" % (tmp, newRes))
            tmp = newRes  # is string

        self.metadata.setMetadataPair(metadata.METADATA_RESOLUTION, float(tmp))
    ...
    def buildTypeCode(self, processInfo):
        ...
        if typecode in WITH_BOUNDINGBOX:
            ...
            tmp = self.metadata.getMetadataValue(metadata.METADATA_SCALE)
