...
class Product_Dimap_Tropforest(Product_Directory):
    ...
    def getDeimosMetadataInfo(self):
        if self.XML_FILE_NAME_MET is None:
            raise Exception(" no metadata file")
        if self.debug != 0:
    ...
    def buildTypeCode(self):
        ...
        elif self.metadata.getMetadataValue(metadata.METADATA_SENSOR_NAME) is None:  # KOMPSAT
    ...
    def refineMetadata(self):
        ...
        if self.metadata.getMetadataValue(metadata.METADATA_START_TIME) is None:
            self.metadata.setMetadataPair(metadata.METADATA_START_TIME, "00:00:00")
        if self.metadata.getMetadataValue(metadata.METADATA_STOP_TIME) is None:
            self.metadata.setMetadataPair(metadata.METADATA_STOP_TIME, "00:00:00")

        # set orbit to 000000 if None
        if self.metadata.getMetadataValue(metadata.METADATA_ORBIT) is None:
        ...
        if tmp is not None and tmp[0] == 'L':
            tmp = tmp[1:]
    ...
    def extractFootprint(self, helper, met):
        ...
        if len(nodes) == 1:
            ...
            try:  # next for non Deimos products
                ...
                met.setMetadataPair(browse_metadata.BROWSE_METADATA_RECT_COORDLIST,
                                    "%s %s %s %s" % (bry, ulx, uly, brx))
                ...
                if len(nodes2) != 0:
                    ...
                    # should be in lower left lat/lon + upper right lat/lon.
                    met.setMetadataPair(browse_metadata.BROWSE_METADATA_RECT_COORDLIST,
                                        "%s %s %s %s" % (second_lat, second_lon, fourth_lat, fourth_lon))
                else:
