...
from worldview_json import __version__, product_worldview, json_emitter

# minimum config version that can be use
MIN_CONFIG_VERSION = 1.0
VERSION = f"Worldview 1-4/Legion converter V:{__version__}"

SIP_REF_NAME='WV1_OPER_WV6_PAN_OR_20100527T231608_N13-756_W100-000_0001_v0100.SIP.ZIP'
EO_REF_NAME='WV1_OPER_WV6_PAN_OR_20100527T231608_N13-756_W100-000_0001'


class ingester_worldview(ingester.Ingester):

        # JSON converter: disable all eoSIP/XML output at the base-ingester level
        # (makeBrowses, SIP/browse/product XML reports, output_eoSip, kmz, sys-items).
        ...
        def afterStarting(self, **kargs):
            # ensure JSON-only mode even if a cfg omits the flag
        ...
        def buildEoNames(self, processInfo, namingConvention=None):
            ...
            previous_platform_id = processInfo.destProduct.metadata.getMetadataValue(metadata.METADATA_PLATFORM_ID)

            if processInfo.srcProduct.is_wv_legion:
                processInfo.destProduct.metadata.setMetadataPair(metadata.METADATA_PLATFORM_ID, 'L')
            self.buildEoNamesDefault(processInfo, namingConvention)
            if processInfo.srcProduct.is_wv_legion:
                processInfo.destProduct.metadata.setMetadataPair(metadata.METADATA_PLATFORM_ID, previous_platform_id)
        ...
        def createSourceProduct(self, processInfo):
            ...
            product = product_worldview.Product_Worldview(processInfo.srcPath)
        ...
        # .SIP.ZIP package. No XML and no ZIP packaging is written.
        # (In normal JSON mode this hook is not reached because output is gated
        # off by TEST_JUST_EXTRACT_METADATA; emission happens in beforeReportsDone.
        # Kept JSON-only so no eoSIP packaging code remains anywhere.)
        #
...
if __name__ == '__main__':
    try:
        if len(sys.argv) > 1:
            ...
            print("Worldview123 conversion report", file=out)
            print(report, file=out)
            print("### End of report", file=out)
            reportName = "Worldview123_conversion_report.txt"
