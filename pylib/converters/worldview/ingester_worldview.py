...
from worldview import __version__, product_worldview
...
class ingester_worldview(ingester.Ingester):
        ...
        def alterReportXml(self, processInfo) :
            ...
            # formattedXml = formattedXml.decode() if type(formattedXml) is bytes else formattedXml


            if processInfo.srcProduct.is_wv_legion:
                formattedXml = formattedXml.split(b'\n')

                for idx_to_remove, line in enumerate(formattedXml):
                    if line.strip().startswith(b'<eop:serialIdentifier>'):

                        break
                else:
                    raise Exception("Cannot find serialIdentifier tag to remove (WorldView Legion)")

                formattedXml.pop(idx_to_remove)
                formattedXml = b'\n'.join(formattedXml)
