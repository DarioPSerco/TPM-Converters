"""JSON metadata emitter for GeoEye-1 products.

Builds a GeoJSON ``Feature`` manifest conformant to
``schema/geoeye1.schema.json``, from a populated eoSip_converter ``Metadata``
object produced by ``product_geoeye1.Product_Geoeye1.extractMetadata``.

STRUCTURE is borrowed from the WorldView EOPF-EOS template
(``schema/worldview.schema.json``). VALUES are GeoEye-1's own — every field is
sourced either from this mission's extraction (the metadata dict) or from this
mission's config ([Mission-specific-values] in ingest_geoeye1.cfg, captured in
``GEOEYE1_PROFILE`` below). NO WorldView values are used. See
``geoeye1_fields.md`` for the per-field mapping and gaps.
...
SCHEMA_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.dirname(os.path.abspath(__file__))))), "schema", "geoeye1.schema.json")

# GeoEye-1 mission profile — values from ingest_geoeye1.cfg [Mission-specific-values].
# (Not read from the metadata object so the emitter also works standalone in tests,
#  where the cfg is not loaded. Source cfg keys noted per line.)
GEOEYE1_PROFILE = {
    "platformShortName": "GeoEye-1",        # cfg METADATA_PLATFORM=GeoEye + METADATA_PLATFORM_ID=1; satId GE01
    "platformSerialIdentifier": 1,          # cfg METADATA_PLATFORM_ID=1
    "sensorType": "OPTICAL",                # cfg METADATA_SENSOR_TYPE=OPTICAL
    "instrumentShortName": "GIS",           # cfg METADATA_INSTRUMENT=GIS
    ...
    "processStepDescription": "EOPF-EOS Converter for GeoEye-1",
    "referenceTitle": "Specialization for GeoEye-1 products v1.2",  # cfg METADATA_SIP_SPEC_NAME_VERSION
    "referenceEdition": "1.2",
    "softwareTitle": "eoProdGen",           # cfg METADATA_SIP_SOFTWARE_NAME=eoProdGen
    "softwareEdition": "0105",              # cfg METADATA_SIP_SOFTWARE_VERSION=0105
    "processorPartyName": "ESA",            # cfg METADATA_RESPONSIBLE=ESA / METADATA_CREATOR=ESA
    "processorPartyType": "CI_Organisation",
}

# GeoEye-1 product types whose geometry is null and which carry a bbox
# (product_geoeye1.WITH_BOUNDINGBOX).
MP_TYPES = {"GIS_PAN_MP", "GIS_4B__MP"}

# generic (non-mission) structural constants — STAC/GeoJSON/ISO, identical for any mission
LINK_MEASUREMENT_TYPE = "image/tiff"        # GeoEye-1 native imagery is GeoTIFF (.TIF)
...
def build_feature(met, product, eo_product_name, profile=GEOEYE1_PROFILE,
                  native_product_name=None):
    """Build the GeoJSON Feature dict for a GeoEye-1 product."""
    ...
    # orbitType: WorldView-schema slot has no GeoEye-1 extraction source -> omitted (gap)
    ...
    # processingLevel: GeoEye-1's own typecode level token (2A / OR / MP) - converter truth, no WV mapping
    level_token = typecode.split("_")[-1] if typecode else None
    if level_token:
        product_info["processingLevel"] = level_token
