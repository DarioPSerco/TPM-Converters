...
from eoSip_converter.esaProducts import product_EOSIP, product_cartosat_image
...
VERSION = "Cartosat-1 Image converter V:0.0.0"
...
#
#
class ingester_cartosat_image(ingester.Ingester):
    ...
    def alterReportXml(self, processInfo):
        ...
        formattedXml = formattedXml.decode() if isinstance(formattedXml, bytes) else formattedXml
    ...
    def createSourceProduct(self, processInfo):
        ...
        product = product_cartosat_image.Product_Cartosat_Image(processInfo.srcPath)
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:
            ingester = ingester_cartosat_image()
            ...
            print("Cartosat-1 image conversion report", file=out)
            print(report, file=out)
            print("### End of report", file=out)
            reportName = "Cartosat-1-Image_conversion_report.txt"
