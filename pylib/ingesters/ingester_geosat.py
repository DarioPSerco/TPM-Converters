...
from eoSip_converter.esaProducts import product_EOSIP, product_geosat
...
VERSION = "Geosat converter V:1.0.0"
SIP_REF_NAME = "DE2_OPER_HRA_PAN_1C_20140515T115211_N41-921_E012-539_0002.SIP.ZIP"
EO_REF_NAME = "DE2_OPER_HRA_PAN_1C_20140515T115211_N41-921_E012-539_0002"

# ALLOWED_SUBTYPES=["PSH", "PM4", "STP"]
ALLOWED_SUBTYPES = ["PAN", "MS4", "PSH", "PS3", "PS4", "PSH", "PM4", "STP"]


#
#
#
class ingester_geosat(ingester.Ingester):
    ...
    def alterReportXml(self, processInfo):
        ...
        if (processInfo.srcProduct.numberOfBrowses) > 1:
            ...
            for item in result2:
                ...
                type = processInfo.srcProduct.metadata.getMetadataValue(
                ...
                if type == "PM4" or type == "STP":
                    ...
                    if subType not in ALLOWED_SUBTYPES:
                        raise Exception(
                            "Unknown browse subtype:%s; shall be one of:%s"
                    ...
                    if subType == "PM4":
                        helper.setNodeAttributeText(
                            nodeSubType,
                            "codeSpace",
                            "urn:esa:eop:GEOSAT2:HiRAIS:Bundle:browsesubType",
                        )
                    else:
                        helper.setNodeAttributeText(
                            nodeSubType,
                            "codeSpace",
                            "urn:esa:eop:GEOSAT2:HiRAIS:Stereo:browsesubType",
                        )
                    helper.setNodeText(nodeSubType, subType)
                    item.insertBefore(nodeSubType, aNodeReferenceSystemIdentifier)
                else:
                    processInfo.addLog(
                        " alterReportXml: METADATA_SENSOR_OPERATIONAL_MODE dont match:%s"
                        % type
                    )
        else:
    ...
    def createSourceProduct(self, processInfo):
        ...
        product = product_geosat.Product_Geosat(processInfo.srcPath)
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:
            ...
            print("Geosat conversion report", file=out)
            print(report, file=out)
            print("### End of report", file=out)
            reportName = "Geosat_conversion_report.txt"
