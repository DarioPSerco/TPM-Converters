...
__version__ = '1.2'
VERSION = "PlanetScope converter V:1.2"
...
class ingester_planetscope(ingester.Ingester):
    ...
    def afterReportsDone(self, processInfo):
        ...
        for root, dirs, files in os.walk(EO_FOLDER, topdown=False):
            ...
            for name in files:
                if root != EO_FOLDER:
                    if determined_typecode not in ("PSC_DEF_OT", "PSC_DEF_S3", "PSC_DEF_S4"):
                        folder_name = os.path.basename(root)
                        if folder_name not in product_planetscope.TYPECODE_REQD_FOLDER_CONTENTS[determined_typecode]:
                            continue
