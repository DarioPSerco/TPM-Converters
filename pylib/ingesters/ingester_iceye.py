...
from eoSip_converter.esaProducts import product_EOSIP, product_iceye
...
VERSION = "ICEYE converter V:1.0.0"
...
class ingester_iceye(ingester.Ingester):
    ...
    def afterReportsDone(self, processInfo):
        ...
        n = 0
        level = processInfo.srcProduct.metadata.getMetadataValue('level')
        for root, dirs, files in os.walk(processInfo.srcProduct.EO_FOLDER, topdown=False):
            for name in files:
                # if level is GRD, ignore all SLC contents
                if level == "GRD":
                    if "slc" in name.lower():
                        continue
                    else:
                        n += 1
                        ...
                        newContentList.append(piece.alias)
                        processInfo.destProduct.addPiece(piece)
                # if level is SLC, ignore all GRD contents
                elif level == "SLC":
                    if "grd" in name.lower():
                        continue
                    else:
                        n += 1
                        aPath = relPath = os.path.join(root, name)
...
if __name__ == '__main__':
    try:
        if len(sys.argv) > 1:
            ...
            print("ICEYE conversion report", file=out)
            print(report, file=out)
            print("### End of report", file=out)
            # print out.getvalue()
            reportName = "iceye_conversion_report.txt"
