...
from eoSip_converter.esaProducts import product_EOSIP, product_worldview123
...
VERSION = "Worldview123 converter V:1.0.0"
...
#
#
class ingester_worldview123(ingester.Ingester):
    ...
    def createSourceProduct(self, processInfo):
        ...
        product = product_worldview123.Product_Worldview123(processInfo.srcPath)
    ...
    def extractMetadata(self, met, processInfo):
        ...
        self.keepInfo(
            "imageDescriptor",
            processInfo.srcProduct.metadata.getMetadataValue("imageDescriptor"),
        )

        self.keepInfo(
            "numberOfBands",
            processInfo.srcProduct.metadata.getMetadataValue("numberOfBands"),
        )

        self.keepInfo(
            "bandId", processInfo.srcProduct.metadata.getMetadataValue("bandId")
        )
        ...
        self.keepInfo(
            metadata.METADATA_CLOUD_COVERAGE,
            processInfo.srcProduct.metadata.getMetadataValue(
                metadata.METADATA_CLOUD_COVERAGE
            ),
        )
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:
            ingester = ingester_worldview123()
