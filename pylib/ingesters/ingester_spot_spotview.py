#
# This is a specialized class that ingest Spot1-5 view dataset
#
...
REF_NAME = (
    "SP5_OPER_HRG__B__1A_20111025T090917_20111025T090926_000000_0092_0232.SIP.ZIP"
)
...
class ingester_spot_spotview(ingester.Ingester):
    ...
    # set the METADATA_BOUNDING_BOX_CW_CLOSED into FOOTPRINT to have 'correct' browse display in EoliSa
    # set use bpunding box for KMZ or not
    #
    def afterProductDone(self, processInfo):
        print(
            " ####################################\n ####################################\nset footprint for shopcart/index to:%s"
    ...
    def extractMetadata(self, met, processInfo):
        ...
        try:
            ...
            met.setMetadataPair(
                metadata.METADATA_WRS_LONGITUDE_GRID_NORMALISED, parentId[1:4]
            )
            met.setMetadataPair(
                metadata.METADATA_WRS_LATITUDE_GRID_NORMALISED, parentId[3:6]
            )
        ...
        if len(self.dataProviders) > 0:
            ...
            for item in list(self.dataProviders.keys()):
                ...
                if item == metadata.METADATA_TRACK:  # fiel is mandatory
                    ...
                    if tmp == None or tmp == metadata.VALUE_NOT_PRESENT:
        ...
        try:
            ...
            processInfo.addLog(
                " #### refine metadata error; metadata: \n%s\n\n\n"
                % processInfo.srcProduct.metadata.toString()
            )
            processInfo.addLog(e)
            raise e
    ...
    def makeBrowses(self, processInfo):
        ...
        try:
            ...
            try:
                ...
                print(
                    "##############@@@@@@@@@@@@@@@@@@@################### warning: can not get product image dimension !!"
                )
                ratio = 33
    ...
    def makeKmz__not_used(self, processInfo):
        if not self.test_dont_write:
            ...
            # use bounding bos except for level 1A
            # NOTE: THIS WILL RESULT IN INCORRECT KMZ, but it is as by the spec
            processLevel = processInfo.srcProduct.metadata.getMetadataValue(
                metadata.METADATA_PROCESSING_LEVEL
            )
            if processLevel == "1A":
                kmzPath = kmz.eosipToKmz.makeKmlFromEoSip_new(
                    False, outPath, processInfo
                )
            else:
                kmzPath = kmz.eosipToKmz.makeKmlFromEoSip_new(
                    True, outPath, processInfo
                )
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:
            ...
            print("Spot spotView conversion report", file=out)
