...
from eoSip_converter.esaProducts import product_EOSIP, product_spot6_7
...
VERSION = "Spot6_7 converter V:1.0.0 2018-10-01. Lavaux Gilles @ Serco"
REF_NAME = "SP6_OPER_NAO_P_S_3__20001122T112233_N20-882_E095-939_0001.SIP.ZIP"


class ingester_spot6_7(ingester.Ingester):
    ...
    def beforeReportsDone(self, processInfo):
        ...
        sensor_mode = processInfo.destProduct.metadata.getMetadataValue(
            metadata.METADATA_SENSOR_OPERATIONAL_MODE
        )
        tmp = sensor_mode
        while sensor_mode.endswith("_"):
            sensor_mode = sensor_mode[0:-1]
        print(
            "@@@@@@@@@@@@@@@@@@@@@@@@@@@@ sensor mode changed from '%s' to '%s'"
            % (tmp, sensor_mode)
        )

        processInfo.destProduct.metadata.setMetadataPair(
            metadata.METADATA_SENSOR_OPERATIONAL_MODE, sensor_mode
        )
    ...
    def afterReportsDone(self, processInfo):
        # for kmz creation: use bounding for level 3
    ...
    def alterReportXml(self, processInfo):
        ...
        mode = processInfo.srcProduct.metadata.getMetadataValue(
            metadata.METADATA_SENSOR_OPERATIONAL_MODE
        )
        ...
        if mode == "P_S":
            processInfo.addLog(
                " alterReportXml: add subType because sensor mode is %s" % mode
            )
            result2 = []
            helper.getNodeByPath(
                entryNode=None,
                path="result/EarthObservationResult/browse/BrowseInformation",
                attr=None,
                result=result2,
            )
            if len(result2) == 0:
                raise Exception(
                    "can not find node 'result/EarthObservationResult/browse/BrowseInformation' in MD report"
                )
            for item in result2:
                aNodeReferenceSystemIdentifier = helper.getFirstNodeByPath(
                    item, "referenceSystemIdentifier", None
                )
                aNodeServiceReference = helper.getFirstNodeByPath(
                    item, "fileName/ServiceReference", None
                )
                ...
                if isMs:
                    ...
                    helper.setNodeAttributeText(
                        nodeSubType,
                        "codeSpace",
                        "urn:esa:eop:NAOMI:Bundle:browsesubType",
                    )
            ...
            processInfo.addLog(
                " alterReportXml: DON'T add subType because sensor mode is %s" % mode
            )
    ...
    def createSourceProduct(self, processInfo):
        ...
        product = product_spot6_7.Product_Spot6_7(processInfo.srcPath)
        if self.test_mode:
            product.setDebug(1)
    ...
    def createDestinationProduct(self, processInfo):
        ...
        if aSize > 1900 * 1024 * 1024:
            processInfo.addLog(
                " ## zip: will use wrapped library because size >=1.9 gb: %s" % (aSize)
            )
            eosipP.setUsePythonZipLib(False)
        else:
            processInfo.addLog(
                " ## zip: will use python library because size <1.9 gb: %s" % (aSize)
            )
        #
    ...
    def extractMetadata(self, met, processInfo):
        ...
        # keep some info
        # spot 6 7 or not
        if processInfo.srcProduct.isSpot:
            ...
            raise Exception("is not a spot product")

        # number of preview
        numPreview = len(processInfo.srcProduct.previewContentName)
        processInfo.addInfo(
            processInfo.srcProduct.origName, "number of preview:%s" % numPreview
        )
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:
            ...
            print("Spot6-7 conversion report", file=out)
            print(report, file=out)
            print("### End of report", file=out)
            # print out.getvalue()
            reportName = "Spot67_conversion_report.txt"
